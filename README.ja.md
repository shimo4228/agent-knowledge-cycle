...
AKC はこれらの failure をこうした運用者から免疫すると主張しない。その防御は訓戒ではなく構造的なものだ。Human Approval Gate ([ADR-0005](docs/adr/0005-human-approval-gate.md)) は遅延ではなく **circuit-breaker (回路遮断器)** である: 「N 日後に自動承認」も「LLM 自身による承認」も経路が存在しないため、ループには divergence を常に止められる地点があり、ゲートの *存在* はループ自身によっては侵食されえない — 人間が注意を払わないことを選んだときだけ侵食される。Curate と Promote は能動的な判断行為であり、サイクルの通常運用そのものが deskilling に抗う行使になっている。これらの limitation は上述のポジティブな framing の正直な双子であり、その撤回ではない。
