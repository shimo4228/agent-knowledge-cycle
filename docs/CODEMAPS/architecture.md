...
| `docs/adr/0028-authority-by-artifact-class-in-the-three-role-loop.md` | Where does merge authority sit in the three-role loop — why admitted task output can merge on a machine gate plus the judge's inspection while behavior-shaping artifacts, the control plane, and the verification machinery stay human? |
...
- **Six phases (held provisionally, ADR-0019)**: Research → Extract → Curate → Promote → Measure → Maintain are the current articulation, unchanged in number, name, and order — but the phase set, the phase-to-skill binding, and the skill set are a mutable snapshot, not a fixed essence (Emptiness axiom). The binding reads "currently scaffolded by X", not "bound bijectively"; Curate is layered (skill-health code layer runs before the semantic layers: skill-stocktake for skills, rules-stocktake for rules, agent-stocktake for agent definitions), and Maintain is layered too (context-sync for documentation roles + repo-asset-stocktake for non-code asset liveness). Concepts can carry scaffolds with the same snapshot status: the human approval gate (human-gate rule + hook, first running instance) and Scaffold Dissolution's generation trigger (generation-audit). Changing the phase set still requires a positioning ADR.
...
README ─────────────────► Why AKC (3 themes) ──────► ADR-N
                                                     ...
                                                     ├──► design-pattern skill repos (how-to companion to ADR-0008 / 0010; externalized)
...
| Category | Count |
|---|---|
| ADRs | ADR-0002–0005, 0008–0028 (canonical count lives in llms-full.txt Project Facts; from 0024 each carries `## Review-when`) |
