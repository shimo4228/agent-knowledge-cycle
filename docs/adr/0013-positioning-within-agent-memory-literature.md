...
- **Curate (prune and refine accumulated memory).** ReMe — *Remember Me, Refine Me* (Cao et al., 2025, arXiv:2512.10696) — is a "dynamic procedural memory framework" whose entire premise is continuously refining what is stored rather than leaving it a "static append-only archive." LangMem (LangChain, 2025) ships memory tools that "control what gets stored," maintain "memory consistency," and "optimize agent behavior through prompt refinement." Both are Curate-and-Promote loops by another name.
...
A 2026 survey, *Externalization in LLM Agents* (Zhou et al., 2026, arXiv:2604.08224), frames the whole field as three distinct but coupled forms of externalization — memory, skills, and protocols — coordinated by harness engineering as the unification layer, in which "capabilities that earlier systems expected the model to recover internally are now externalized." AKC is squarely inside that frame. Pretending otherwise would be a false novelty claim, the kind ADR-0011 was written to prevent.
...
**Correction (2026-06-07).** Direct verification of the primary texts during position-paper sourcing found three characterization drifts in this ADR, corrected in place: ReMe's premise was quoted as "beyond static memory approaches" (phrase not present in the primary text; its actual vocabulary is "static append-only archive" / "passive accumulation"); LangMem's mechanism was quoted as "prompt updates" (the primary text says "optimize agent behavior through prompt refinement"); and the Zhou et al. survey was characterized as "four coupled forms of externalization — memory, skills, protocols, and harness engineering" (the survey names three forms — memory, skills, protocols — and explicitly states harness engineering "is not a fourth kind of externalization" but the unification layer that coordinates them). The decision itself — concede, then locate — is unaffected.

**Addendum (2026-06-25): the skill-library-maintenance literature.** A second wave of 2026 work studies the same operations one layer down — the *skill library* as the unit of maintenance, which is exactly AKC's Curate and Maintain phases. SkillOps (Pu, Song & Zhao 2026, arXiv:2605.13716) frames "skill technical debt" and a four-dimension library-health diagnosis, and proposes a **self-maintaining** ecosystem that prunes, validates, and de-duplicates skills autonomously. SoK: Agentic Skills (Jiang et al. 2026, arXiv:2602.20867) maps a seven-stage skill lifecycle (discovery → practice → distillation → storage → retrieval → execution → evaluation/update) — a field map for the skill layer the way the Externalization survey is for the broader field. The in-the-wild benchmarking study (Liu et al. 2026, arXiv:2604.04323) supplies the empirical counterpart: skill benefits are *fragile*, degrading toward the no-skill baseline as a library grows large and uncurated.

The concession and the delta are unchanged, only sharpened. The operations — prune, validate, de-duplicate, retire — are again not novel; SkillOps names them as a maintenance discipline. AKC's delta is the same human approval gate: SkillOps's defining adjective is *self-maintaining* (the loop closes without a human in the write path), where AKC's Curate and Promote require named human sign-off (ADR-0005). The in-the-wild study is the most useful of the three because it is *corroboration rather than precedent*: its finding that uncurated skill accumulation does not help, and can hurt, is independent empirical support for ADR-0010's claim that curation is the load-bearing, attention-bound act — the scarce resource is the human judgment that decides what stays, not the storage that holds it. As the Negative consequence above predicted, these entrants strengthen the concession rather than invalidating it.

**Correction (2026-06-30).** The 2026-06-25 addendum above originally called Liu et al. (arXiv:2604.04323) "SkillsBench." That nickname was wrong and is removed: Liu et al.'s paper is titled *How Well Do Agentic Skills Work in the Wild* and does not name its benchmark SkillsBench, while a separately-authored paper actually titled *SkillsBench* (Li et al. 2026, arXiv:2602.12670) is a different work. The citation (arXiv:2604.04323) and the finding it grounds — that skill benefits weaken toward the no-skill baseline as the library grows large and uncurated — are correct; only the label was. Both primaries verified 2026-06-30. (The graph node's internal `@id` slug `prior-art/skillsbench` is left unchanged to preserve its `sameAs` identity; its `name` field already carries the correct title.)

**Addendum (2026-07-08): the concept-corroboration wave.** A third wave of citations, different in kind from the addenda above: not precedent to concede, and not field maps to locate against, but external work independently converging on concepts this repository had already named. They are recorded so that each concept carries its external evidence in the knowledge graph — and so the research pipeline stops re-surfacing the same convergences as gaps. All seven primaries were verified against the arXiv API (title, authorship, abstract) on 2026-07-08.

- *Runtime architecture patterns* (Srinivasan 2026, arXiv:2605.20173) names the **stochastic-deterministic boundary (SDB)** — a four-part contract of proposer, verifier, commit step, and reject signal — as "the load-bearing primitive of production agent runtimes": a production-derived formalization of **code-LLM layering** (ADR-0008), where code decides and the LLM proposes.
- *Context Engineering 2.0* (Hua et al. 2026, arXiv:2510.26493) treats the selection and shaping of what reaches the model as a discipline in its own right, with a history predating LLMs — independent convergence with **signal-first** and **cognitive economy** (ADR-0010).
- *The Cognitive Divergence* (Eliav 2026, arXiv:2603.26707) quantifies the asymmetry those two concepts assume — model context windows expanding roughly 3,906-fold (2017–2026) while human effective context span contracts — and names a self-reinforcing **delegation feedback loop**, the same coupling ADR-0014 records as delegation-feedback divergence.
- *SkillC* (Lin et al. 2026, arXiv:2605.27899) converts the gap between skill-injected and skill-free rollouts into a direct learning signal for skill internalization — **scaffold dissolution** operationalized as a measurable, driveable process rather than an end-state aspiration.
- *When Agents Go Quiet* (Agyemang et al. 2026, arXiv:2604.16736) proves a Format-Cost Separation Theorem — deferred rendering is at least as token-efficient as direct generation for any format whose overhead multiplier exceeds one — turning ADR-0004's **two-stage distill pipeline** from an empirical rule into a conditionally optimal design.
- *The verification-bottleneck study* (Huemmer et al. 2026, arXiv:2601.17055) is a three-wave longitudinal record of the **loop failure modes** running in a human population: participants leaned on AI most heavily for the hardest tasks, exactly where their verification confidence and accuracy were lowest — the bidirectional growth loop observed running in reverse.
- *Agent security as agent-human interaction* (Wang et al. 2026, arXiv:2605.24309) analyzes 59 academic papers against 21 production agent systems and finds the academically favored mechanisms (intent anchoring, trust labeling) at zero production deployment, while human-centric mechanisms — policy specification, runtime approval, scope configuration — dominate industry practice: the **intent alignment** theme's industry–academia gap, measured.

The relation is the one the 2026-06-25 addendum named for Liu et al.: corroboration rather than precedent. Nothing here is conceded and no delta moves; these works are the external evidence that the named concepts pick out real, independently observed structure.

**Addendum (2026-07-23): controlled ability-transfer corroboration (EvoAgentBench).** A fourth kind of entry: not precedent, not a field map, and stronger than the observational corroborations above — the first *controlled comparison* between curated and automatically extracted skill content on the axis where the human-gate delta lives. EvoAgentBench (Gao et al. 2026, arXiv:2607.05202) benchmarks agent self-evolution as ability transfer under an **ability-supported yet instance-disjoint** split: every test task is backed by verified training-side ability support, but no test task repeats a training instance (528/267 train/test), across 4 domains × 2 scaffolds × 3 backbones = 24 evaluation cells. Curated ability content — the curator-side "Anchor Skill" reference, ability-grounded skills with deterministic retrieval, explicitly *not a deployable method* — yields positive per-domain gain in **all 24 cells**; no automatic extraction method evaluated (Memento, ReasoningBank, GEPA) sustains positive gain in all settings, with negative transfer reaching **−36.3 points** (Memento, software engineering, one scaffold-backbone cell). One honesty note bounds the reading: the curated condition is an oracle-style upper reference that includes deterministic retrieval, so the result supports "human-curated content transfers where autonomous extraction does not reliably" as a controlled observation — not proof that any human gate suffices. Where Liu et al. (2026-06-25 addendum) showed skill benefits are fragile as libraries grow uncurated, EvoAgentBench shows the sharper, controlled version: the fragility is a *transfer* phenomenon, invisible to same-distribution evaluation. The same evidence carries a tension side, recorded where it belongs: it contradicts the sufficiency of the dissolution Completion Criterion's same-context ablation test, and ADR-0022 (experimental) revises that criterion to require held-out transfer as the completion evidence. Primary verified against the arXiv record (title, abstract, Table 3 values) on 2026-07-23.
