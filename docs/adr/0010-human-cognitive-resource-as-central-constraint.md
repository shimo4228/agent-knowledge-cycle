...
**Origin loop.** The `daily research` report that triggered this ADR was generated by a pipeline the author had been running for months — the same pipeline that, in an earlier session, surfaced Laukkonen et al. (2025)'s *Contemplative Inference* paper, which seeded the Contemplative Agent research project, whose engineering substrate became the prior art for ADR-0002 through ADR-0007. The signal-first Research principle that ADR-0010 names was being practiced by the pipeline that eventually produced the report that named it. The principle was lived before it was stated; this ADR is the point at which the practice recognized itself. See [`../inspiration.md`](../inspiration.md) for the full chain.
