...
### 2. Cognitive economy as Design Principle #8
...
This sits alongside the existing principles (Composable, Observable, Non-destructive, Tool-agnostic, Evaluation scales with model capability, Scaffold dissolution, Code-LLM Layering).
...
**Origin loop.** The `daily research` report that triggered this ADR was generated by a pipeline the author had been running for months — the same pipeline that, in an earlier session, surfaced Laukkonen et al. (2025)'s *Contemplative Inference* paper, which seeded the Contemplative Agent research project, whose engineering substrate became the prior art for several early ADRs (the ones that remain in AKC are ADR-0002 through ADR-0005; see [`../inspiration.md`](../inspiration.md)). The signal-first Research principle that ADR-0010 names was being practiced by the pipeline that eventually produced the report that named it. The principle was lived before it was stated; this ADR is the point at which the practice recognized itself. See [`../inspiration.md`](../inspiration.md) for the full chain.
