...
1. **Name the three-role loop — judge / build / human — a mechanism-level concept of the cycle**: the operational form the human approval gate takes under attention scarcity. The judge tier spends model judgment to conserve human judgment (premise verification, worth-doing verdicts, dispatch, independent acceptance); the build tier implements; the human holds only direction and the final switch. The gate stays binary and human at the point of merge — what moves upstream to the judge tier is the attention the triage step consumes, not the authority the gate holds.
   ...
   > **Note (2026-09-25, ADR-0028)**: The human-held merge switch in this clause is retired for admitted task output outside the protected class ADR-0028 names (behavior-shaping artifacts, the control plane, and the verification machinery). Since 2026-09-15 the operator's harness merges such output once a deterministic gate and the judge tier's independent inspection both pass, and the human's authority over it sits at admission (which tasks exist, on what acceptance conditions) and in a revert-count expiry — see [ADR-0028](./0028-authority-by-artifact-class-in-the-three-role-loop.md). What stands: the topology (the judge verifies, decides worth-doing, dispatches, and independently inspects; the build tier implements and halts on a refuted premise; the human sets direction), and the human-held merge for everything [ADR-0005](./0005-human-approval-gate.md) gates. The "merge" and "final switch" wording elsewhere in this ADR (Summary, Context, Decision 2, Consequences, Relationship to other ADRs) reads under this note.
