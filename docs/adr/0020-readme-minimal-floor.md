...
1. **README.md moves to a minimal-floor structure.** It keeps only: (a) a short lead with definition, tagline, audience, and companion-paper DOI; (b) a single "Why AKC" section — the *only* full presentation of the three themes, in the fixed order cognitive resource → intent alignment → the cycle changes the human too; (c) the cycle diagram with the single phase→skill table; (d) install instructions; (e) a compact repo-contents table including the AI-navigation row; (f) short Limitations and Positioning paragraphs that point to the governing ADRs; (g) merged Origin & Acknowledgments carrying the author identifiers (GitHub handle, ORCID); (h) How to Cite, opening with the concept-vs-release DOI distinction; (i) compact Related Work; (j) License. (A standalone "Facts" table was part of the first cut of this rewrite; it was dissolved the same day — its only non-duplicated rows were the author identifiers, the DOI-line distinction, and the AI-navigation pointers, which moved into (g), (h), and (e) respectively. The remaining rows duplicated the lead, the BibTeX block, and the License section.)
