...
In the author's harness the same distinction runs as an evaluation loop for
...
an LLM judge only screens fidelity to the source, and the author's blind
reading decides what is worth reading.
...
|---|---|---|---|
| **Procedures** | Step-by-step skills an agent loads on demand — the phase table below. Scaffolding by design: meant to dissolve once internalized | the phase table below; [harness-boundary](https://github.com/shimo4228/claude-harness/tree/main/skills/harness-boundary) asks, before a mechanism is added, whether the next model generation will make it unnecessary | [ADR-0019](docs/adr/0019-cycle-structure-is-provisional.md), [Scaffold Dissolution](docs/scaffold-dissolution.md) |
...
change that shapes future behavior lands without a named human sign-off. In the
running harness the gate is one always-loaded rule,
